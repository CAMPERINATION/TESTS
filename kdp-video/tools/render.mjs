#!/usr/bin/env node
// Frame-exact render of the animation to MP4 (1080x1920, H.264 + AAC) with the voiceover muxed in.
//
//   node tools/render.mjs                      -> out/kdp-service-short.mp4
...
const out = path.resolve(root, opt('out', 'out/kdp-service-short.mp4'));
const audio = path.resolve(root, opt('audio', 'audio/voiceover.wav'));
...
const types = { '.html': 'text/html', '.js': 'text/javascript', '.ttf': 'font/ttf', '.wav': 'audio/wav', '.mp3': 'audio/mpeg' };
...
const { total } = await page.evaluate(() => window.KDP_READY);
...
const ffArgs = [
  ...
  '-ss', String(from), '-i', audio,
  '-map', '0:v', '-map', '1:a',
  '-c:v', 'libx264', '-preset', 'medium', '-crf', '18', '-pix_fmt', 'yuv420p', '-r', String(fps),
  '-c:a', 'aac', '-b:a', '192k', '-af', `apad=whole_dur=${(to - from).toFixed(3)}`,
...
for (let i = 0; i < n; i += BATCH) {
  ...
  const urls = await page.evaluate((ts) => ts.map((t) => window.renderFrameJPEG(t, 0.93)), times);
  for (const u of urls) {

...
## Prompt + Media Intake UI

Use the bundled intake UI when the user wants to provide their own prompt, A-roll, and voice-over:

```bash
bash scripts/serve-prompt-ui.sh 4173
```

Then open `http://localhost:4173`.
The UI now submits multipart form data to a local API, saves uploaded A-roll/voice-over files, and returns a persisted brief JSON response.
Saved files are stored under `assets/prompt-ui/data/uploads/` and briefs under `assets/prompt-ui/data/briefs/`.
Use the returned stored paths to copy assets into your Remotion project (for example, `public/media/`) and bind them in scene components.
...
├── remotion.config.ts
├── package.json
└── assets/prompt-ui/       # Prompt + A-roll + voice-over intake UI (includes local upload API server)
```
